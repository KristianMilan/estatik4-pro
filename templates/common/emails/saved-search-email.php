<h1 style="font-size: 23px; font-family: 'Open Sans'; font-weight: 400; color: rgb(66, 66, 66);"><?php _e( 'These new homes match your saved search', 'es-plugin' ); ?>:</h1>
					...
					<table cellspacing="0" cellpadding="0" border="0" width="100%" style="border: 1px solid #ebeaea;">
						<tr>
							<td class="property-thumbnail">
                                <img alt="<?php es_the_title(); ?>" src="<?php echo es_get_the_featured_image_url( 'es-image-size-archive', true ); ?>"/>
							</td>
						</tr>
						<tr>
							<td style="padding: 20px;">
								...
								<table style="padding-top: 8px; padding-bottom: 15px; margin-left: -3px;">
									<?php if ( $address = es_get_the_address() ) : ?>
										<tr>
											<td colspan="2" style="font-size: 12px; font-weight: 200; font-family: 'Open Sans'; color: rgb(108, 108, 108);"><?php _e( 'Address', 'es-plugin' ); ?>: <?php echo $address; ?></td>
										</tr>
									<?php endif; ?>
									<?php if ( $property->bedrooms || $property->bathrooms ) : $cols = 0; ?>
										<tr>
											<?php if ( $property->bedrooms ) : $i++; ?>
												<td style="font-size: 12px; font-weight: 200; font-family: 'Open Sans'; color: rgb(108, 108, 108);"><?php _e( 'Bedrooms', 'es-plugin' ); ?>: <?php echo $property->bedrooms; ?></td>
											<?php endif; ?>
											<?php if ( $property->bathrooms ) : $i++ ?>
												<td style="font-size: 12px; font-weight: 200; font-family: 'Open Sans'; color: rgb(108, 108, 108); "><?php _e( 'Bathrooms', 'es-plugin' ); ?>: <?php echo $property->bathrooms; ?></td>
											<?php endif; ?>
									...
									<?php if ( $area = es_get_the_formatted_field( 'area' ) ) : ?>
										<tr>
											<td colspan="2" style="font-size: 12px; font-weight: 200; font-family: 'Open Sans'; color: rgb(108, 108, 108);"><?php _e( 'Area', 'es-plugin' ); ?>: <?php echo $area; ?></td>
										</tr>
									<?php endif; ?>
								</table>
								...
								<a href="<?php echo get_permalink( $post ); ?>" style="background-color: <?php echo ests( 'main_color' ); ?>; border-radius: 2px; font-size: 14px; font-family: 'Open Sans'; color: rgb(255, 255, 255); line-height: 2.5; text-decoration: none; display: block; width: 100%; text-align: center;" target="_blank">
									<?php _e( 'View More Details', 'es-plugin' ); ?>
								</a>
							</td>
						</tr>
					</table>
...
<?php if ( ( $search_url = es_get_search_page_url() ) && ( $search_data = $saved_search->search_data ) ) : ?>
	<table style="width: 100%; margin-top: 20px;" cellpadding="0" cellspacing="0" border="0">
		<tr>
			<td>
				<a style="border:1px solid <?php echo ests( 'main_color' ); ?>; border-radius: 2px; font-size: 14px; font-family: 'Open Sans'; color: <?php echo ests( 'main_color' ); ?>; line-height: 2.5; text-decoration: none; display: block; width: 100%; text-align: center;" target="_blank" href="<?php echo add_query_arg( $search_data, $search_url ); ?>"><?php _e( 'See All', 'es-plugin' ); ?></a>
			</td>
		</tr>
	</table>
<?php endif;
