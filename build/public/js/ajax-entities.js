( function( $ ) {
                ...
                if ( $search_forms.length ) {
                    $search_forms.each( function() {
                        ...
                        $search_form.on( 'change', 'input:not([type=text]):not([type=number]):not([type=search]),select', function() {
                            ...
                            var hash = new EntitiesHash( $field.closest( '.js-es-entities-wrap' ).find( '.js-es-entities' ).data( 'hash' ) );
                        ...
                        $search_form.on( 'submit', function() {
                            var $wrapper = $search_form.closest( '[data-search-form-selector]' ).find( '.js-es-entities__wrap_inner' );
                            ...
                            xhr = $.get( Estatik.settings.ajaxurl, {action: 'get_' + entity_type, hash: hash.getHash(), reload_map: 1 }, function( response ) {
                                response = response || {};
    ...
    var EntitiesHash = function( hash, is_temp ) {
        ...
        var _this = this;
        this.hash = hash;
        this.attributes = new URLSearchParams( window.atob( this.hash ) );
        ...
        EntitiesHash.prototype.clearQueryArguments = function() {
            ...
            this.tempEntitiesHash.getAttributes().forEach( function( value, key ) {
                if ( ! temp_hash.shortcode_attributes.includes( key ) ) {
                    _this.delete( key );
                }
            } );
        };
         ...
         */
        EntitiesHash.prototype.appendValue = function( key, value ) {
            _this.attributes.append( key, value );
            _this.hash = btoa( _this.attributes.toString() );
    ...
    var HalfMap = function( $wrapper ) {
        ...
        HalfMap.prototype.loadListingsFromVisibleBounds = function() {
            ...
            if ( _this.markers.length ) {
                ...
                _this.xhr = $.get( Estatik.settings.ajaxurl, { reload_map: 0, hash: hash.getHash(), action: 'get_listings' }, function( response ) {
        ...
        HalfMap.prototype.propertyPopup = function( location, property_config ) {
            return function() {
                ...
                _this.xhr = $.get( Estatik.settings.ajaxurl, request_data, function( response ) {
                    response = response || {};
    ...
    $( function() {
        ...
        $( document ).on( 'click', '.js-es-pagination a.page-numbers', function() {
            ...
            if ( ! $pagination_wrapper.find( '.page-numbers--preload' ).length ) {
                ...
                hash.setValue( 'paged-' + hash.getValue( 'loop_uid' ), page );
                hash.setHistoryQuery();
        ...
        $( document ).on( 'click', '.js-es-remove-filters', function() {
            ...
            if ( $wrapper.find( '.js-es-search-nav__reset' ).length ) {
                $wrapper.find( '.js-es-search-nav__reset' ).trigger( 'click' );
            } else {
                $wrapper.find( '.js-es-search input, js-es-search select' ).val(null).trigger( 'change' );
            }

            // setTimeout( function() {
            //     $btn.removeClass( 'es-btn--preload' ).removeAttr( 'disabled' ).removeProp( 'disabled' );
            // }, 4000 );
            return false;
        } );
