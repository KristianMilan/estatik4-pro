( function( $ ) {
                                    ...
                                    if ( ( breakpoints.hasOwnProperty( className ) && breakpoints[ className ].min > container_width ) || isMaxBreakpoint ) {
                                        ...
                                        if ( 'single-tiled-gallery' === layout || 'single-slider' === layout ) {
                                            if ( className === 'es-single--xl' ) {
                                                $container.find( '.js-es-control .es-btn' )
                                                    .removeClass( 'es-btn--icon' )
                                                    .addClass( 'es-btn--big' )
                                                    .removeClass( 'es-btn--medium' );
                                            } else {
    ...
    $( function() {
        ...
        $( document ).on( 'click', '.js-es-search [type=reset]', function() {
            var $form = $( this ).closest( 'form' );

            $form.find( '.es-field, .js-es-field' ).find( 'input, select' )
                .val( null )
                .attr( 'value', '' )
                .attr( 'data-value', '' )
                .trigger( 'change' );
        ...
        $( document ).on( 'click', '.js-es-save-listing', function() {
            var $form = $( this ).closest( 'form' );
                ...
                if ( prices_list && prices_list.length ) {
                    for ( var i in prices_list ) {
                        if ( prices_list[i].category === value_category && prices_list[i].type === value_type ) {
                            var min_list = prices_list[i].min_prices_list.split( ',' );
                            var max_list = prices_list[i].max_prices_list.split( ',' );

                            $price_min.html('');
                            $price_max.html('');

                            for ( var j in min_list ) {
                                $price_min.append( new Option( min_list[j], min_list[j] ) );
                            }

                            for ( var k in max_list ) {
                                $price_max.append( new Option( max_list[k], max_list[k] ) );
                            }
