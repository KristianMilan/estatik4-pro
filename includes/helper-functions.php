...
function es_field_builder_field_render( $field_key, $field_config, $field ) {
	...
	$field_config = es_parse_args( $field_config, array(
		...
		'value'      => maybe_unserialize( $field[ $field_key ] ),
	) );
...
function es_field_builder_field_option_render( $field_key, $is_attribute = false, $field_config = array(), $field = array() ) {
	...
	if ( $is_attribute ) {
		$value = isset( $options['attributes'][ $field_key ] ) ? $options['attributes'][ $field_key ] : '';
	} else {
...
function es_get_pdf_sections() {
	...
	foreach ( array( 'description', 'location', 'media', 'video' ) as $section_id ) {
...
function es_get_recursive_parent_locations( $term_id, $parent_id = array() ) {
	if ( $term_id ) {
		$parent_id[] = $term_id;
		$parent = get_term_meta( $term_id, 'parent_component', false );
		if ( $parent ) {
...
function es_get_listing_meta_icons_fields() {
	...
	$fields = $area_fields + $numeric_fields + $text_fields + $price_fields + $tax_fields + $inc_fields + $ltsz_fields;
