...
abstract class Es_Entity {
	...
	public function get_field_value( $field ) {
		...
		if ( ! empty( $f_info['value_callback'] ) ) {
			if ( ! empty( $value['value_callback'][0] ) && function_exists( $value['value_callback'][0] ) ) {
				if ( ! empty( $value['value_callback'][1] ) ) {
					$value = call_user_func_array( $value['value_callback'][0], $value['value_callback'][1] );
				} else {
					$value = call_user_func( $value['value_callback'][0] );
				}
			}
