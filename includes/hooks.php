	...
	function es_privacy_policy( $context ) {
		...
		if ( $terms_forms && is_array( $terms_forms ) && in_array( $context, $terms_forms ) ) {
			...
			if ( 'checkbox' == ests( 'terms_input_type' ) ) {
				$content = __( 'I agree to the Terms of Use and Privacy Policy', 'es' );
		...
		if ( ! $context || ( is_array( $forms ) && in_array( $context, $forms ) ) ) {
			...
			if ( $site_key && $site_secret ) {
				...
				if ( 'v3' == $recaptcha_version ) {
					...
					<?php
				} else if ( 'v2' == $recaptcha_version ) : $args['onload'] = 'es_initialize_recaptcha'; ?>
 ...
 * @return string
 */
function es_get_avatar( $avatar, $id_or_email, $size, $default, $alt ) {
	$user = false;
	...
	if ( $user && is_object( $user ) && ( $entity = es_get_user_entity( $user->ID ) ) ) {
		...
		$avatar = "<img alt='{$alt}' src='{$src}' class='avatar avatar-{$size}' width='{$size}' height='{$size}'/>";
	}

	return $avatar;
}
add_action( 'get_avatar', 'es_get_avatar', 10, 5 );
...
function es_stats_counter( $entity_id ) {
    ...
    <div class="es-stats__item es-stats__item--active">
        ...
        <span><a href="<?php echo es_get_the_permalink(); ?>#listings"><?php printf( _n(
					'property',
					'properties',
					$entity->get_property_qty()
				), $entity->get_property_qty() ); ?></a></span>
    </div>
    </div><?php
}
	...
	if ( ! empty( ests( 'recaptcha_site_key' ) ) ) {
		if ( is_array( $forms ) && in_array( 'comment_form', $forms ) ) {
			if ( ! isset( $_POST['g-recaptcha-response'] ) || empty( $_POST['g-recaptcha-response'] ) ) {
				wp_die( __( 'ReCaptcha Field is Required', 'es-plugin' ) );
			}
...
function es_cron_new_schedules( $schedules ) {
	...
	if ( empty( $schedules['daily'] ) ) {
		$schedules['daily'] = array(
			'interval' => 86400,
			'display'  => esc_html__( 'Daily', 'es-plugin' ),
		);
	}
	...
	if ( empty( $schedules['weekly'] ) ) {
		$schedules['weekly'] = array(
			'interval' => 604800,
			'display'  => esc_html__( 'Weekly', 'es-plugin' ),
		);
	}
	...
	if ( empty( $schedules['monthly'] ) ) {
		$schedules['monthly'] = array(
			'interval' => 2635200,
			'display'  => esc_html__( 'Monthly', 'es-plugin' ),
		);
	}
	...
	if ( empty( $schedules['15m'] ) ) {
		$schedules['15m'] = array(
			'interval' => 900,
			'display'  => esc_html__( 'Each 15 min', 'es-plugin' ),
		);
	}
	...
	if ( empty( $schedules['10m'] ) ) {
		$schedules['10m'] = array(
			'interval' => 600,
			'display'  => esc_html__( 'Each 10 min', 'es-plugin' ),
		);
	}
	...
	if ( empty( $schedules['5min'] ) ) {
		$schedules['5min'] = array(
			'interval' => 300,
			'display'  => esc_html__( 'Each 5 min', 'es-plugin' ),
		);
	}
	...
	if ( empty( $schedules['2min'] ) ) {
		$schedules['2min'] = array(
			'interval' => 120,
			'display'  => esc_html__( 'Each 2 min', 'es-plugin' ),
		);
	}
...
function es_user_has_request_cap( $allcaps, $caps, $args ) {
	if ( ! empty( $args[2] ) && in_array( $args[0], array( 'read_post', 'edit_post', 'delete_post' ) ) ) {
		$user_id = get_post_meta( $args[2], 'es_request_user_id', true );

		if ( get_post_type( $args[2] ) === Es_Request::get_post_type_name() && $user_id == $args[1] ) {
			switch ( $args[0] ) {
				case 'read_post':
					$allcaps['read'] = true;
					break;
...
function es_render_property_pdf() {
	...
	if ( $query->have_posts() ) {
		...
		$contact_fields = apply_filters( 'es_pdf_contact_fields', array(
			__( 'Phone', 'es-plugin' ) => ests( 'pdf_phone' ),
			__( 'Email', 'es-plugin' ) => ests( 'pdf_email' ),
			__( 'Address', 'es-plugin' ) => ests( 'pdf_address' ),
		) );
...
function es_agents_query_alter( $query ) {
	...
	if ( current_user_can( 'agent' ) && is_admin() ) {
		/** @var WP_Query $query */
	...
	if ( $id && es_get_entity_by_id( $id ) && in_array( $post->post_type, es_builders_supported_post_types() ) ) {
		...
		if ( $elementor_editor_mode || $divi_builder ) {
			$fields['alternative_description'] = array(
				...
				'order' => -8,
				'label' => __( 'Alt Description', 'es-plugin' ),
